...
{
    list_node_t *p;
    list_node_t *find = list_find(head_p, val);
    if (find != NULL)
    {
        return find;
    }
...
list_node_t *list_insert_delete_dup2(list_node_t *head_p, int val)
{
    ...
    while (current != NULL)
    {
        if (current->val == val)
        {
            list_node_t *next_node = current->next; // 次のノードを保存
            if (prev == NULL)
            {
                // 削除するノードが先頭にある場合
                head_p = next_node;
    ...
    // ノードが見つからなかった場合、新しいノードを挿入
    return list_insert(head_p, val);
}
...
{
    list_node_t *current = head_p->next;
